---
title: "Gráficas Variables Cuantitativas."
Autor: Carlos Alberto Ramírez Cázares
format: 
  html:
    grid: 
      body-width: 950px
...
municipios_Pueblao <- st_read("C:\Users\Ch4rl\OneDrive\Escritorio\UNAM 5\Estadistica Aplicada\Tarea 1\DATA\21mun.shp", options = "ENCODING=LATIN1", quiet = TRUE)

# Crear mapa de municipios de Puebla
mapa_municipios <- ggplot(municipios_Pueblao) +
  geom_sf(fill = "lightgreen", color = "white", size = 0.3) +
  labs(
    title = "Municipios de Puebla",
    subtitle = paste("Total de municipios:", nrow(municipios_Pueblao)),
...
Extraer el archivo `cpv_valor_21.csv` y guardarlo en la carpeta `data/`.
...
datos_total <- read_csv("C:\Users\Ch4rl\OneDrive\Escritorio\UNAM 5\Estadistica Aplicada\Tarea 1\DATA\cpv_valor_21.csv", na = c("ND"))
...
datos <- left_join(municipios_Pueblao, datos)
```
...
ggplot(datos) +
  ...
  labs(
    title = "Derechohabientes IMSS (%) de los Municipios de Puebla",
    caption = "Fuente: INEGI - Marco Geoestadístico"
  ) +
  theme_void() +
  theme(
...
ggplot(data = datos, aes(sample = porcentaje_de_la_poblacion_derechohabiente_en_el_imss)) +
    ...
    labs(title = "Gráfico Q-Q para Normalidad",
    subtitle = "Derechohabientes IMSS (%) - Puebla") +
    theme_bw()+
    theme(
    legend.title = element_text(size=13, face="bold"),
    legend.text = element_text(size=13),
...
ggplot(datos) +
  ...
  labs(
    title = "Población de los Municipios de Puebla",
    caption = "Fuente: INEGI - Marco Geoestadístico"
  ) +
  theme_void() +
  theme(
