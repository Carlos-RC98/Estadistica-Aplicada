...
Se toman en cuenta los datos de finanzas públicas estatales contenidos en los archivos CSV de la carpeta *conjunto_de_datos*, excluyendo la información correspondiente a la **Ciudad de México**.
...
Seleccionamos las variables numéricas y de un año en particular (2020).
...
Se observa una marcada heterogeneidad regional en los patrones de egreso estatal. Las diferencias responden a prioridades presupuestarias, niveles de ingreso, población y desarrollo económico. Al mismo tiempo, algunos estados muestran estructuras de gasto similares, posiblemente influenciadas por su contexto geográfico y socioeconómico

*Agrupamiento jerarquico*

Se aplicó un **agrupamiento jerárquico** con base en la matriz de distancias euclidianas, mostrando los **dendrogramas** obtenidos mediante distintos métodos de enlace.
...
mapa <- ggplot(nacional_estado) +
  geom_sf(aes(fill=grp_euc_complete), color = "gray", size = 0.5) +
  labs(
...
mapa <- ggplot(nacional_estado) +
  geom_sf(aes(fill=grp_euc_ward2), color = "gray", size = 0.5) +
  labs(
