---
title: "DUD (DISTRIBUCIÓN UNIFORME DISCRETA)"
format: html
editor: visual
---

```{r}
#| label: setup
#| include: false
library(tidyverse)
library(kableExtra)
library(gridExtra)
```

  

La distribución uniforme discreta es una de las distribuciones de probabilidad más importantes y see caracteriza porque todos los valores posibles de la variable aleatoria tienen la misma probabilidad de ocurrir.

Funcion de probabilidad

$$
\begin{equation}
f(x) = P(X=x) = \begin{cases}
\frac{1}{n} & \ text{si} x \in\{x_1, x_2, \ldots, x_n} \\
0 & \text{otro caso}
\end{cases}
\end{equation}
$$

Probabilidad acumulada

Si los valores $x_1, x_2, \ldots, x_n$ son $n$ enteros consecutivos:

$$
\begin{equation}
F(x) = P(X\leq x)= \begin{cases}
0 & \text {si} x < x_1 \\
\frac{\lfloor x \rfloor - x_1 +1}{x_n - x_1+1 & \text{si} x_1 \leq x \leq x_n\\
1 & \text{si} x > x_n
\end{cases}
\end{equation}
$$

Esperanza media:

$$
\begin{equation}
E(X) = \frac{1}{n}\sum_{i=1}^n x_1 =\mu
\end{equation}
$$

Varianza:

$$
\begin{equation}
\text{Var}(X) = {1}{n}\sum_{i=1}^n (x_i-\mu)^2
\end{equation}
$$

Aplicacion:

::: {#exm-dado}
Un dado justo representa una distribución $X\sim \text{unif} \{1, 2, 3, 4, 5, 6\}$.

```{r}
#| code-fold: true
#| label: dado-ejemplo

# Parámetros del dado
x_1 <- 1
x_6 <- 6
valores <- x_1:x_6
probabilidades <- rep(1/length(valores), length(valores))

# Crear data frame
dado_data <- data.frame(
  Valor = valores,
  Probabilidad = probabilidades
)

# Mostrar tabla
kable(dado_data, 
      caption = "Distribución de probabilidad de un dado",
      align = "cc") %>%
  kable_styling(bootstrap_options = "striped")
```

Los parámetros:

```{r}
#| code-fold: true
#| label: parametros-dado
# Media
media <- sum(dado_data$Valor)/length(dado_data$Valor)
cat("Media:", media, "\n")

# Varianza
varianza <- sum((dado_data$Valor- media)^2)/length(dado_data$Valor)
cat("Varianza:", varianza, "\n")

# Desviación estándar
desv_std <- sqrt(varianza)
cat("Desviación estándar:", round(desv_std, 4), "\n")
```

Visualización:

```{r}
#| code-fold: true
#| label: grafico-dado
#| fig-cap: "Función de probabilidad de un dado"

ggplot(dado_data, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "steelblue", alpha = 0.7, width = 0.6) +
  geom_text(aes(label = round(Probabilidad, 3)), vjust = -0.5) +
  scale_x_continuous(breaks = valores) +
  scale_y_continuous(limits = c(0, 0.20)) +
  labs(title = "Distribución Uniforme Discreta - Dado",
       x = "Valor",
       y = "Probabilidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
```
:::

::: {#exm-num_random}
### Selección aleatoria de números

Si seleccionamos aleatoriamente un número entero entre 10 y 20.

**Cálculo de parámetros:**

```{r}
#| code-fold: true
#| label: ejemplo-numeros

x2_1 <- 10
x2_n <- 20
valores2 <- x2_1:x2_n
probabilidades2 <- rep(1/(length(valores2)), length(valores2))

# Data_frame
data2 <- data.frame(
  Valor = valores2,
  Probabilidad = probabilidades2
)

# Parámetros estadísticos
media2 <- mean(data2$Valor)
varianza2 <- sum((data2$Valor - media2)^2)/length(data2$Valor)
desv_std2 <- sqrt(varianza2)

cat("Media:", media2, "\n")
cat("Varianza:", round(varianza2, 4), "\n")
cat("Desviación estándar:", round(desv_std2, 4), "\n")
```

Funcion de probabilidad:

```{r}
#| label: grafico-numeros
#| fig-cap: "Distribución uniforme discreta entre 10 y 20"


ggplot(data2, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "green", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 3)), 
            vjust = -0.5, size = 3) +
  scale_x_continuous(breaks = seq(10, 20, 1)) +
  labs(title = "Distribución Uniforme Discreta",
       x = "Valor",
       y = "Probabilidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))
```

Simulación

Generación de números aleatorios de una variable aleatoria.

$X\sim unif\{1,2,3,4,5,6\}$ y comparación de las probabilidades.

```{r}
#| code-fold: true
#| label: simulacion

# Generar 1000 números aleatorios
set.seed(123)
n_sim <- 1000
muestra <- sample(1:6, n_sim, replace = TRUE)

# Frecuencias observadas
freq_obs <- table(muestra)
freq_rel <- prop.table(freq_obs)

# Comparar con probabilidades teóricas
comparacion <- data.frame(
  Valor = 1:6,
  Prob_Teorica = rep(1/6, 6),
  Freq_Observada = as.numeric(freq_rel)
)

kable(comparacion, 
      caption = "Comparación entre probabilidades teóricas y observadas",
      col.names = c("Valor", "Prob. Teórica", "Freq. Observada"),
      digits = 4) |> 
  kable_styling(bootstrap_options = "striped")
```

Visualización de la simulación:

```{r}
#| code-fold: true
#| label: grafico-simulacion
#| fig-width: 8
#| fig-height: 6
#| fig-align: "center"
#| fig-cap: "Comparación entre distribución teórica y simulada"


# Preparar datos para el gráfico
comp_long <- comparacion |> 
  pivot_longer(cols = c(Prob_Teorica, Freq_Observada),
               names_to = "Tipo",
               values_to = "Probabilidad") |> 
  mutate(Tipo = case_when(
    Tipo == "Prob_Teorica" ~ "Teórica",
    Tipo == "Freq_Observada" ~ "Observada"
  ))

ggplot(comp_long, aes(x = Valor, y = Probabilidad, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.7) +
  scale_x_continuous(breaks = 1:6) +
  scale_fill_manual(values = c("Teórica" = "lightblue", "Observada" = "darkred")) +
  labs(title = paste("Simulación con", n_sim, "observaciones"),
       x = "Valor",
       y = "Probabilidad",
       fill = "Distribución") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "bottom")
```
:::

Aplicaciones:

La distribución uniforme discreta tiene múltiples aplicaciones:

1.  **Juegos de azar**: Dados, ruletas, sorteos
2.  **Muestreo**: Selección aleatoria de elementos
3.  **Simulación**: Generación de números aleatorios equiprobables
4.  **Investigación**: Asignación aleatoria de tratamientos

## Propiedades Importantes

### Relación con otras distribuciones

-   Caso especial de la distribución categórica con probabilidades iguales
-   Límite discreto de la distribución uniforme continua
-   Base para generar otras distribuciones discretas

### Teorema Central del Límite

Verifiquemos el Teorema Central del Límite mediante simulación para el caso de variables aleatorias con distribución uniforme discreta.

Generamos $k=1000$ muestras de tamaño $n=30$ cada una, de la distribucion uniforme discreta en el conjunto $\{1,2,3,4,5,6\}$ podemos observar la distribucion de sus medias muestrales. En el ejemplo del dado se observo que para $X\sim {unif}\{1,2,3,4,5,6\}$ tenemos $\mu= 7/2$ y $\sigma^2 = 35/12$.

$$
\bar{X}_{30}=\frac{1}{30}\sum_{i=1}^{30} X_i$$ \$\$

Lo que sera una aproximacion de la distribución:

$$\bar{X}_n \sim N\left(\mu, \frac{\sigma^2}{n} \right)$$\$\$

$$\bar{X}_{30} \sim N\left(\frac{7}{2}, \frac{7}{72} \right)$$\$\$

```{r}
#| code-fold: true
#| label: limite-central
#| fig-width: 8
#| fig-height: 6
#| fig-align: "center"
#| fig-cap: "Verificación del Teorema del Límite Central"
# Simular medias muestrales
set.seed(456)
k_muestra <- 1000
n_muestra <- 30

medias_muestrales <- replicate(k_muestra, {
  muestra <- sample(1:6, n_muestra, replace = TRUE)
  mean(muestra)
})

# Crear histograma
hist_data <- data.frame(Medias = medias_muestrales)

ggplot(hist_data) +
  geom_histogram(aes(x=Medias, after_stat(density)), bins = 30, 
                 fill = "lightgreen", alpha = 0.7, color = "red") +
  geom_density(aes(x = Medias), color = "orange", linewidth = 1) +
  geom_vline(xintercept = mean(medias_muestrales), 
             color = "lightblue", linetype = "dashed", linewidth = 1) +
  labs(title = "Distribución de Medias Muestrales",
       subtitle = paste("n =", n_muestra, " de una v.a. uniforme(1,6)"),
       x = "Media Muestral",
       y = "Densidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5))

# Estadísticas
cat("Media teórica:", (1+6)/2, "\n")
cat("Media observada:", round(mean(medias_muestrales), 4), "\n")
cat("Varianza teórica de la media:", round(((6-1+1)^2-1)/12/n_muestra, 4), "\n")
cat("Varianza observada de la media:", round(var(medias_muestrales), 4), "\n")
```

Ejercicio:

::: {#exr-muestras}
Una urna contiene 8 bolas numeradas del 1 al 8. Se extrae una bola al azar. Calcule la probabilidad de obtener un número par.

```{r}
#| label: exr-muestras-solucion
#| code-fold: true

# Parámetros
x_1 <- 1
x_n <- 8
valores <- x_1:x_n
n <- length(valores)
probabilidad <- 1/n

# Crear data frame de distribución
distribucion_ex1 <- data.frame(
  Valor = valores,
  Probabilidad = rep(probabilidad, n),
  Es_Par = valores %% 2 == 0
)

# Cálculo de la probabilidad P(X es par)
prob_par <- sum(distribucion_ex1$Probabilidad[distribucion_ex1$Es_Par == TRUE])

# Mostrar tabla de distribución
# library(kableExtra)
kable(distribucion_ex1, 
      caption = "Distribución de probabilidad de la urna (1 a 8)",
      align = "ccc",
      digits = 4) %>%
  kable_styling(bootstrap_options = "striped")

# Resultado de la probabilidad
cat("Probabilidad de obtener un número par:", prob_par, "\n")
```

La probabilidad de tener un numero par al escoger una pelota de la urna es de \$0.5\$ (\$50\\%\$) por los 4 resultados pares \$\\{2,4,6,8\\}\$ de 8 resultados posibles.
:::

<div>

En el juego se selecciona un numero entero entre 5 y 15. Calcular la media y var de esta distribución:

```{r}
#| label: exr-enteros-solucion
#| code-fold: true
#| fig-cap: "Distribución uniforme discreta entre 5 y 15"

# Definir el rango de números
x2_1 <- 5
x2_n <- 15
valores2 <- x2_1:x2_n
n2 <- length(valores2)
probabilidad2 <- 1/n2

# Data_frame
data2 <- data.frame(
  Valor = valores2,
  Probabilidad = rep(probabilidad2, n2)
)

# Parámetros estadísticos
media2 <- mean(data2$Valor)
varianza2 <- sum((data2$Valor - media2)^2)/n2

# Visualización de la función de probabilidad
# library(ggplot2)
ggplot(data2, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "darkgreen", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 4)), 
            vjust = -0.5, size = 3) +
  scale_x_continuous(breaks = valores2) +
  labs(title = "Distribución Uniforme Discreta (5 a 15)",
       x = "Valor",
       y = "Probabilidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))

cat("Media E(X):", media2, "\n")
cat("Varianza Var(X):", varianza2, "\n")

```

</div>

::: {#exr-simulacion_4}
Simule 500 observaciones de una distribución uniforme discreta en {0, 1, 2, 3, 4} y compare las frecuencias observadas con las teóricas.

```{r}
#| label: exr-simulacion_4-solucion
#| code-fold: true
#| fig-cap: "Comparación entre distribución teórica y simulada (n=500)"

# Parámetros de la simulación
valores_sim <- 0:4
n_sim <- 500
prob_teorica <- 1 / length(valores_sim)

# Generar 500 números aleatorios
set.seed(42) # Usando la misma semilla para reproducibilidad
muestra_sim <- sample(valores_sim, n_sim, replace = TRUE)

# Frecuencias observadas
freq_obs <- table(factor(muestra_sim, levels = valores_sim)) # Asegura que 0:4 estén presentes
freq_rel_obs <- prop.table(freq_obs)

# Comparar con probabilidades teóricas
comparacion_sim <- data.frame(
  Valor = valores_sim,
  Prob_Teorica = rep(prob_teorica, length(valores_sim)),
  Freq_Observada = as.numeric(freq_rel_obs)
)

# Mostrar tabla de comparación
# library(kableExtra)
kable(comparacion_sim, 
      caption = "Comparación entre probabilidades teóricas y observadas (n=500)",
      col.names = c("Valor", "Prob. Teórica", "Freq. Observada"),
      digits = 4) %>%
  kable_styling(bootstrap_options = "striped")

# Preparar datos para el gráfico
# library(tidyverse)
comp_long <- comparacion_sim %>% 
  pivot_longer(cols = c(Prob_Teorica, Freq_Observada),
               names_to = "Tipo",
               values_to = "Probabilidad") %>% 
  mutate(Tipo = case_when(
    Tipo == "Prob_Teorica" ~ "Teórica",
    Tipo == "Freq_Observada" ~ "Observada"
  ))

# Visualización de la simulación
# library(ggplot2)
ggplot(comp_long, aes(x = Valor, y = Probabilidad, fill = Tipo)) +
  geom_col(position = "dodge", alpha = 0.7) +
  scale_x_continuous(breaks = valores_sim) +
  scale_fill_manual(values = c("Teórica" = "lightblue", "Observada" = "darkred")) +
  labs(title = paste("Simulación con", n_sim, "observaciones"),
       x = "Valor",
       y = "Probabilidad",
       fill = "Distribución") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        legend.position = "bottom")
```
:::
