...
densidad_t <- ggplot(df_data, aes(x = x, y = density, color = distribution)) +
  ...
  scale_color_manual(values = c("firebrick", "lightlightblue", "cyan", "orange3", "black")) +
  labs(
...
p1 <- ggplot(df_cdf, aes(x = x, y = cdf, color = distribution, linetype = distribution)) +
  geom_line(linewidth = 1.2) +
  scale_color_manual(values = c("firebrick", "lightblue", "cyan", "orange3", "black")) +
...
p_pdf <- ggplot(df_pdf, aes(x = x, y = pdf, color = distribution)) +
  geom_line(linewidth = 1.2) +
  scale_color_manual(values = c("firebrick", "cyan", "orange3", "black")) +
  labs(
...
create_probability_plot <- function(df_val, a_val, b_val = NULL, prob_type, title_text) {
  ...
  p <- ggplot(df_base, aes(x = x, y = y)) +
    ...
    theme(
      ...
      plot.subtitle = element_text(hjust = 0.5, size = 10, color = "lightblue")
    )
  
  # Agregar área sombreada según el tipo
  if (prob_type == "left") {
    p <- p + geom_area(data = data.frame(x = x_area, y = y_area), 
                       aes(x = x, y = y), fill = "skylightblue", alpha = 0.7) +
      geom_vline(xintercept = a_val, color = "firebrick", linetype = "dashed", linewidth = 1)
  } else if (prob_type == "right") {
    p <- p + geom_area(data = data.frame(x = x_area, y = y_area), 
    ...
    p <- p + geom_area(data = data.frame(x = x_area, y = y_area), 
                       aes(x = x, y = y), fill = "lightcyan", alpha = 0.7) +
      geom_vline(xintercept = c(a_val, b_val), color = "firebrick", linetype = "dashed", linewidth = 1)
  } else if (prob_type == "two_tails") {
    p <- p + geom_area(data = data.frame(x = x_area1, y = y_area1), 
                       aes(x = x, y = y), fill = "orange3", alpha = 0.7) +
      geom_area(data = data.frame(x = x_area2, y = y_area2), 
                aes(x = x, y = y), fill = "orange3", alpha = 0.7) +
      geom_vline(xintercept = c(-abs(a_val), abs(a_val)), color = "firebrick", linetype = "dashed", linewidth = 1)
  }
...
p_var <- ggplot(df_variance, aes(x = n, y = variance)) +
  ...
  geom_hline(yintercept = 1, color = "firebrick", linetype = "dashed", size = 1) +
  labs(
...
hist(muestra_grande, breaks = 30, probability = TRUE, col = "lightlightblue",
     main = paste("Distribución t(5) vs Normal(0,1) — n =", n_max),
     xlab = "x", ylab = "Densidad")

# Densidad t(5)
curve(dt(x, df = gl), from = -5, to = 5, add = TRUE, col = "firebrick", lwd = 2)

# Densidad Normal estándar
curve(dnorm(x, mean = 0, sd = 1), from = -5, to = 5, add = TRUE, col = "cyan", lwd = 2, lty = 2)

legend("topright",
       legend = c("Muestra simulada", "t(5) teórica", "Normal(0,1)"),
       col = c("lightlightblue", "firebrick", "cyan"),
...
# --- 2. Visualización ---
hist(T_values, breaks = 40, probability = TRUE, col = "lightlightblue",
     main = expression(paste("Simulación del estadístico  T = (X̄ - μ) / (S / √n)")),
     xlab = "T", ylab = "Densidad")

# Superponer la densidad teórica t(n-1)
curve(dt(x, df = n - 1), from = -5, to = 5, add = TRUE, col = "firebrick", lwd = 2)
...
hist(T_values, breaks = 30, probability = TRUE, col = "lightlightblue",
