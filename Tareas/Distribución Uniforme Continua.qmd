...
pdf_graf <- ggplot(df_pdf) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        legend.position = "top") +
  scale_color_manual(values = c("darkred", "lightblue", "green"))
...
ggplot(df_ejemplo, aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = c(c_ej, d_ej), linetype = "dashed", color = "orange") +
...
ggplot(df_convergencia, aes(x = n)) +
  geom_line(aes(y = media_acumulada), color = "lightblue", alpha = 0.7) +
  geom_hline(aes(yintercept = media_teorica), color = "orange", linetype = "dashed", linewidth = 1) +
...
ggplot(data.frame(x = muestra_hist), aes(x = x)) +
  ...
  geom_hline(yintercept = 1/(b-a), color = "orange", size = 2, linetype = "dashed") +
...
ggplot(data.frame(tiempo = tiempos_espera), aes(x = tiempo)) +
  ...
  geom_vline(xintercept = tiempo_teorico, color = "darkred", size = 1, linetype = "dashed") +
  geom_vline(xintercept = 10, color = "lightblue", size = 1, linetype = "dotted") +
...
ggplot(data.frame(diametro = diametros), aes(x = diametro)) +
  ...
  geom_vline(xintercept = diametro_objetivo, color = "darkred", size = 1) +
...
ggplot(df_tcl, aes(x = medias)) +
  ...
  geom_vline(xintercept = media_poblacional, color = "darkred", size = 1, linetype = "dashed") +
...
ggplot(estadisticos_tcl, aes(x = n)) +
  ...
  scale_color_manual(values = c("Observada" = "lightblue", "Teórica" = "darkred")) +
...
ggplot(data.frame(medias = medias_n100), aes(sample = medias)) +
  geom_qq() +
  geom_qq_line(color = "darkred", size = 1) +
...
abline(v = c(79, 89), col = "darkred", lwd = 2, lty = 2)
text(x = c(74.5, 84, 94.5), y = max(table(calificaciones))/2,
     labels = c("Aprobado","Bien","Excelente"), col = "yellow", cex = 0.9)
...
for (n in n_values) {
  ...
  hist(medias_muestrales, breaks = 30, probability = TRUE, col = "green",
       main = paste("TCL - n =", n), xlab = "Media muestral", ylab = "Densidad")
  
  # Superponer densidad normal teórica
  mu_med <- mean(x_trunc)
  sigma_med <- sd(x_trunc)/sqrt(n)
  curve(dnorm(x, mean = mu_med, sd = sigma_med), add = TRUE, col = "darkred", lwd = 2)
...
# --- Visualización ---
plot(x[dentro_circulo], y[dentro_circulo], col = "lightblue", pch = 16, cex = 0.5,
     xlab = "x", ylab = "y", asp = 1, main = "Estimación de pi con Monte Carlo")
points(x[!dentro_circulo], y[!dentro_circulo], col = "darkred", pch = 16, cex = 0.5)
legend("topright", legend = c("Dentro del círculo", "Fuera del círculo"),
       col = c("lightblue", "darkred"), pch = 16)
