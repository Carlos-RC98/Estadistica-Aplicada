...
1. **Efecto del parámetro $a$ (grados de libertad del numerador)**: A medida que $a$ aumenta, la distribución se vuelve más simétrica y se desplaza hacia nuestra derecha, lo que nos acerca a una forma más "normal".

2. **Efecto del parámetro $b$ (grados de libertad del denominador)**: Al aumentar $b$, la distribución se concentra más a su media y firebrickuce su variabilidad. También la cola derecha se vuelve menos pesada.
...
4. **Asimetría**: La última gráfica demuestra que $F(a, b) \neq F(b, a)$, mostrandonos la no conmutatividad de los parámetros.
...
Esta relación nos ayuda a calcular valores críticos y cuantiles en tablas estadísticas.
...
Se presentan las gráficas de la función de distribución acumulada para diferentes combinaciones de grados de libertad:
...
La media de una variable aleatoria $X \sim F(a, b)$ existe cuando $b > 2$ y está dada por:
...
p1 <- ggplot(p1_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media1, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda1, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media1 + 0.3, y = max(p1_data$y) * 0.9, 
           label = paste0("Media = ", round(media1, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda1 - 0.3, y = max(p1_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda1, 3)), color = "green", angle = 90) +
...
p2 <- ggplot(p2_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media2, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda2, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media2 + 0.2, y = max(p2_data$y) * 0.9, 
           label = paste0("Media = ", round(media2, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda2 - 0.2, y = max(p2_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda2, 3)), color = "green", angle = 90) +
...
p3 <- ggplot(p3_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media3, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda3, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media3 + 0.15, y = max(p3_data$y) * 0.9, 
           label = paste0("Media = ", round(media3, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda3 - 0.15, y = max(p3_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda3, 3)), color = "green", angle = 90) +
...
p4 <- ggplot(p4_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media4, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda4, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media4 + 0.2, y = max(p4_data$y) * 0.9, 
           label = paste0("Media = ", round(media4, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda4 - 0.2, y = max(p4_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda4, 3)), color = "green", angle = 90) +
...
p5 <- ggplot(p5_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media5, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda5, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media5 + 0.15, y = max(p5_data$y) * 0.9, 
           label = paste0("Media = ", round(media5, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda5 - 0.15, y = max(p5_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda5, 3)), color = "green", angle = 90) +
...
p6 <- ggplot(p6_data, aes(x = x, y = y)) +
  geom_line(linewidth = 1, color = "lightcyan") +
  geom_vline(xintercept = media6, linetype = "dashed", color = "firebrick", linewidth = 0.8) +
  geom_vline(xintercept = moda6, linetype = "dashed", color = "green", linewidth = 0.8) +
  annotate("text", x = media6 + 0.15, y = max(p6_data$y) * 0.9, 
           label = paste0("Media = ", round(media6, 3)), color = "firebrick", angle = 90) +
  annotate("text", x = moda6 - 0.15, y = max(p6_data$y) * 0.9, 
           label = paste0("Moda = ", round(moda6, 3)), color = "green", angle = 90) +
...
4. **Convergencia**: Para valores grandes de ambos parámetros (esquina superior derecha), la varianza se acerca a cero, indicando que la distribución se concentra más alfirebrickedor de su media.
...
hist(muestras_f, breaks = 50, probability = TRUE, col = "steelblue",
     main = expression(paste("(a)  Si  X ~ t(n)  =>  X"^2," ~ F(1,n)")),
     xlab = "x", xlim = c(0, 10))
curve(df(x, df1 = 1, df2 = n), col = "firebrick", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(1,n) teórica"),
       col = c("steelblue", "firebrick"), lwd = c(10, 2), bty = "n")
...
hist(muestras_inv, breaks = 50, probability = TRUE, col = "lightgreen",
     ...
     xlab = "x", xlim = c(0, 5))
curve(df(x, df1 = b, df2 = a), col = "firebrick", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(b,a) teórica"),
       col = c("lightgreen", "firebrick"), lwd = c(10, 2), bty = "n")
...
hist(muestras_F2, breaks = 50, probability = TRUE, col = "orange",
     main = expression(paste("(c)  (X/a)/(Y/b)  ~  F(a,b)")),
     xlab = "x", xlim = c(0, 5))
curve(df(x, df1 = a, df2 = b), col = "firebrick", lwd = 2, add = TRUE)
legend("topright", legend = c("Simulación", "F(a,b) teórica"),
       col = c("orange", "firebrick"), lwd = c(10, 2), bty = "n")
