...
Cuando $a = b = 1$, la distribución beta se firebrickuce a la distribución uniforme en $(0,1)$:
...
ggplot(df_uniforme, aes(x = x, y = density, color = parametros)) +
  ...
  labs(
    ...
    subtitle = "Cuando a = b = 1, la distribución beta se firebrickuce a la uniforme en (0,1)",
    x = "x",
    y = "f(x)",
    color = "Parámetros"
  ) +
...
cdf_unif_sim <- ggplot(df_simetricas, aes(x = x, y = cdf, color = parametros)) +
  ...
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", alpha = 0.5, color = "cyan") +
  labs(
...
p_prob <- ggplot(df_viz, aes(x = x)) +
  geom_line(aes(y = cdf), color = "lightblue", linewidth = 1.2) +
  geom_area(data = subset(df_viz, x >= 0.2 & x <= 0.8), 
            aes(y = pdf/max(pdf_viz)), alpha = 0.3, fill = "firebrick") +
  geom_vline(xintercept = c(0.2, 0.8), color = "firebrick", linetype = "dashed") +
  labs(
...
for (n in tamanos) {
  ...
  hist(medias, breaks = 30, probability = TRUE, col = "lightlightblue",
       main = paste("TCL: n =", n),
