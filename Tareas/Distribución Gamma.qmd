...
colores <- c("firebrick", "purple", "green", "blue")
...
ggplot(df_cdf_efectos, aes(x = x, y = y, color = parametro)) +
  ...
  labs(title = "Función de Distribución Acumulativa - Distribución Gamma",
       ...
       x = "x", y = "F(x) = P(X ≤ x)",
       color = "Parámetro") +
  scale_color_brewer(palette="Paifirebrick")+
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5)) +
  ylim(0, 1)
...
ggplot(data.frame(x = x_supinf, y = pdf_supinf), aes(x = x, y = y)) +
  ...
  geom_vline(xintercept = c(a_val, corte_superior), 
             linetype = "dashed", color = c("green", "yellow"), linewidth = 1) +
  ...
  annotate("label", x = corte_superior + 1, y = max(pdf_supinf) * 0.3,
           ...
           size = 4, color = "yellow", fontface = "bold") +
...
ggplot(data.frame(x = x_ejemplo, y = pdf_ejemplo), aes(x = x, y = y)) +
  geom_line(linewidth = 1.5, color = "purple") +
  geom_vline(xintercept = media_ej, color = "firebrick", linewidth = 1.2, linetype = "solid") +
  ...
  geom_vline(xintercept = c(media_ej - desv_std_ej, media_ej + desv_std_ej), 
             color = "yellow", linewidth = 1, linetype = "dotted") +
  annotate("text", x = media_ej + 0.2, y = max(pdf_ejemplo) * 0.8,
           label = paste("Media =", round(media_ej, 2)), color = "firebrick", size = 4) +
  ...
  annotate("text", x = media_ej + desv_std_ej + 0.2, y = max(pdf_ejemplo) * 0.4,
           label = paste("μ ± σ"), color = "yellow", size = 4) +
...
p1 <- ggplot(df_convergencia, aes(x = n)) +
  geom_line(aes(y = media_acumulada), color = "purple", alpha = 0.8) +
...
ggplot(data.frame(x = muestra_grande), aes(x = x)) +
  geom_histogram(aes(y = after_stat(density)), bins = 50, 
                 fill = "lightpurple", color = "black", alpha = 0.7) +
  ...
  geom_vline(xintercept = median(muestra_grande), color = "purple", linetype = "dotted", linewidth = 1) +
...
for (n in n_sizes) {
  ...
  plots_tcl[[as.character(n)]] <- ggplot(data_plot, aes(x = Medias)) +
    geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "yellow", alpha = 0.7, color = "black") +
