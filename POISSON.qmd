...
ggplot(data_ejemplo, aes(x = k, y = probabilidad, fill = color)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Normal" = "#BDa1a2", 
                              "P(X = 6)" = "#E35C3C", 
                              "P(X = 0)" = "#3808DB",
                              "P(X ≤ 3)" = "#2ECc22")) +
...
El parámetro $\lambda$ controla la posición central yla dispersión de la distribución de Poisson.
...
ggplot(data_comp, aes(x = k, y = Probabilidad, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Teórica" = "#2348DB", "Simulada" = "#E21C3C")) 
...
ggplot(data_suma_comp, aes(x = valores, fill = metodo)) +
  ...
  theme_minimal() +
  theme(legend.position = "top") +
  scale_fill_manual(values = c("X1 + X2" = "#7548DB", "Poisson(λ1+λ2)" = "#E11C3C"))
```
...
ggplot(data_multiple, aes(x = valores, fill = tipo)) +
  ...
  labs(title = "Suma de Múltiples Variables Poisson",
       ...
       fill = "Tipo") +
  theme_minimal() +
  scale_fill_manual(values = c("#a2CC23", "#d32C12"))
```
...
ggplot(data_tcl, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.7, fill = "#6432DB") +
  geom_vline(xintercept = lambda_tcl, color = "#A43C3C", linetype = "dashed", linewidth = 1) +
...
plots_comparacion <- lapply(names(data_tcl_split), function(label) {
  ...
  ggplot(datos, aes(x = media_muestral)) +
    geom_histogram(aes(y = after_stat(density)), bins = 25, alpha = 0.6, fill = "#6432DB") +
    stat_function(fun = dnorm, 
                  args = list(mean = lambda_tcl, sd = sqrt(lambda_tcl/n_val)), 
                  color = "#A43C3C", linewidth = 1.2) +
    ggtitle(label) +
    xlab("Media muestral") +
    ylab("Densidad") +
    theme_minimal()
...
ggplot(data_trafico, aes(x = paquetes, y = probabilidad, fill = zona)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Bajo" = "#2FCA21", "Normal" = "#6432DB", 
                              "Alto" = "#C12", "Sobrecarga" = "#AD123C"))
```
...
ggplot(data_calidad, aes(x = defectos, fill = clasificacion)) +
  ...
  labs(title = "Simulación de Control de Calidad",
       ...
       fill = "Clasificación") +
  theme_minimal() +
  scale_fill_manual(values = c("Perfecto" = "#2FCA21", "Aceptable" = "#6432DB",
                              "Revisión" = "#C12", "Rechazo" = "#AD1C3C"))
...
ggplot(data_epi, aes(x = casos, y = probabilidad, fill = escenario)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = c("Sin intervención" = "#ad123C", "Con intervención" = "#2fca21"))
```
